...
then
  ...
  "$BUILDLOCATION"/debootstrap/debootstrap --merged-usr --keyring "$BUILDLOCATION"/debootstrap/keyrings/debian-archive-keyring.pgp --arch "$BUILDARCH" "$DEBIANRELEASE" "$BUILDLOCATION"/build/"$BUILDARCH"/$PHASE1_PATHNAME $DEBIANREPO
  debootstrapresult=$?
...
then
  ...
  "$BUILDLOCATION"/debootstrap/debootstrap --merged-usr --keyring "$BUILDLOCATION"/debootstrap/keyrings/debian-archive-keyring.pgp --arch "$BUILDARCH" "$DEBIANRELEASE" "$BUILDLOCATION"/build/"$BUILDARCH"/$PHASE2_PATHNAME $DEBIANREPO
  debootstrapresult=$?
  if [[ $debootstrapresult == 0 ]]
  then
