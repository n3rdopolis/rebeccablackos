...
then
  ...
  "$BUILDLOCATION"/debootstrap/debootstrap --arch "$BUILDARCH" bullseye "$BUILDLOCATION"/build/"$BUILDARCH"/$PHASE1_PATHNAME $DEBIANREPO
  debootstrapresult=$?
...
then
  ...
  "$BUILDLOCATION"/debootstrap/debootstrap --arch "$BUILDARCH" bullseye "$BUILDLOCATION"/build/"$BUILDARCH"/$PHASE2_PATHNAME $DEBIANREPO
  debootstrapresult=$?
  if [[ $debootstrapresult == 0 ]]
  then
