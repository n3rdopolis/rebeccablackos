...
checkinstall -y -D --fstrans=no --nodoc --dpkgflags=--force-overwrite --install=yes --backup=no --pkgname=rbos-rbos --pkgversion=1 --pkgrelease=$(date +%s)  --maintainer=rbos@rbos --pkgsource=rbos --pkggroup=rbos --requires="expect,whois,dlocate,xterm,vpx-tools,screen,kbd,checkinstall,acl,xdg-utils" /tmp/configure_phase3_helper.sh
cd $OLDPWD
...
REMOVEDEVPGKS=(texlive-base gnome-user-guide cmake libgl1-mesa-dri-dbg libglib2.0-doc valgrind smbclient freepats libc6-dbg doxygen git subversion bzr mercurial texinfo autoconf cpp-5 gcc-5 g++-5 unicode-data)
