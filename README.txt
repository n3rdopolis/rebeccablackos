...
BUILDING: 
     ...
     If you need to select specific revisions of the packages, copy buildcore_revisions_(architechture).txt into ~/RBOS_Build_Files . Ensure that the file is official, and not tampered with, as it is *executed* by the build scripts to set the revision. Only revision files built by SVN commit 3418 are fully compatible for the packages. Revisions files can also specify a snapshot date for the underlying Debian packages, but only since commit 5555.
     ...
     CONTROL FILES (relative to ~/RBOS_Build_Files):
        ...
        DontForceSnapshotBuild(architechture): Delete this file only after the first run is complete, before the next build. This forces temporary chroots to be built
