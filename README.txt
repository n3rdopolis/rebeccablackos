...
This has both Wayland and X. Even though QT, GTK, EFL, and SDL has been compiled on this CD to support Wayland, under these toolkits, many apps don't work.
...
How to use Wayland:
	...
	-Wayland attempts to start a  Wayland server running "bare metal" as the display server. This is not supported on all machines, especially ones with closed source drivers.

	-Xorg Backed Waylandruns a fullscreen wayland session with X as the display server. There isn't really a performance penalty, but Wayland won't manage the layout of the screens
